#pragma once

#include <unistd.h> // UNIX standard function definitions
#include <chrono>  // time
#include <termios.h> // POSIX terminal control definitionss

constexpr uint8_t RADIO_ADDRESS = 0xEA;
constexpr uint8_t ADDR_MODULE = 0xEE;  //  Crossfire transmitter
constexpr uint8_t TYPE_CHANNELS = 0x16;

// internal crsf variables
constexpr int RC_MIN = 988;
constexpr int RC_MID = 1500;
constexpr int RC_MAX = 2012;

constexpr int CRSF_MIN = 172;
constexpr int CRSF_MAX = 1811;
constexpr speed_t BAUDRATE = B921600;             // Crossfire/ELRS typically uses 400000
constexpr std::chrono::microseconds CRSF_TIME_NEEDED_PER_FRAME_US(1100);
constexpr std::chrono::milliseconds FAILSAFE_TIMEOUT(1000);
//   Hz     ms
//   25     40000
//   50     20000
//   100    10000
//   250    4000
//   500    2000 (???)
constexpr int CRSF_TIME_BETWEEN_FRAMES_US = 4000; // 4 ms 250Hz
constexpr int CRSF_MAX_CHANNEL = 16;
constexpr int CRSF_FRAME_SIZE_MAX = 64;
constexpr int SERIAL_BAUDRATE = 400000;
constexpr int CRSF_MSP_RX_BUF_SIZE = 128;
constexpr int CRSF_MSP_TX_BUF_SIZE = 128;
constexpr int CRSF_PAYLOAD_SIZE_MAX   = 60;
constexpr int CRSF_PACKET_LENGTH = 22;
constexpr int CRSF_PACKET_SIZE  = 26;
constexpr int CRSF_FRAME_LENGTH = 24;   // length of type + payload + = crc;
constexpr int CRSF_CHANNEL_BITS = 11;

// named channels of radio
enum chan_order{
   ROLL,
   PITCH,
   THROTTLE,
   YAW,
   AUX1,  // (CH5)  ARM switch for Expresslrs
   AUX2,  // (CH6)  angel / airmode change
   AUX3,  // (CH7)  flip after crash
   AUX4,  // (CH8)
   AUX5,  // (CH9)
   AUX6,  // (CH10)
   AUX7,  // (CH11)
   AUX8,  // (CH12)
   AUX9,  // (CH12)
   AUX10,  // (CH12)
   AUX11,  // (CH12)
   AUX12,  // (CH12) // telemetry?
};

// crc implementation from CRSF protocol document rev7
constexpr uint8_t crsf_crc8tab[256] = {
};

